<template>
  <q-page class="q-pa-md">
    <div class="row items-center justify-between q-mb-md">
      <div>
        ...
        <p class="text-subtitle2 text-grey-7 q-mb-none">
          Gestiona, busca y filtra la información de los colaboradores en tiempo real.
        </p>
      </div>
    </div>
    ...
    <q-card class="q-mb-md q-pa-md bg-grey-1" flat bordered>
      ...
      <div class="row q-col-gutter-sm">
        ...
        <div class="col-12 q-mt-sm">
          <div class="text-caption text-grey-10 text-weight-medium">
            Rango de Edad: {{ filters.ageRange.min }} - {{ filters.ageRange.max }} años
          </div>
          <q-range
    ...
    <DetalleUsuario ref="modalDetalleRef" />
  </q-page>
</template>
...
import DetalleUsuario from 'src/components/DetalleUsuario.vue' // Importamos tu nuevo componente

// Referencia al componente QDialog
...
function filterCompanyFn(val, update) {
  update(() => {
    searchCompany.value = val.toLowerCase()
  })
}
function filterCityFn(val, update) {
  update(() => {
    searchCity.value = val.toLowerCase()
  })
}
function filterCountryFn(val, update) {
  update(() => {
    searchCountry.value = val.toLowerCase()
  })
}
...
const verDetalleUsuario = (user) => {
  ...
  // Llamamos a la función de tu componente para abrir el modal con el ID correspondiente
  modalDetalleRef.value.abrirDetalle(user.id)
}
</script>
